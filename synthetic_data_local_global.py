...
Step 6: Stack on top of {target_object} location at {target_pos} with height {target_height} + 5 and open gripper
"""
...
def generate_task(task_type):
    ...
    source_z = random.choice([17, 18, 19])
    ...
    if task_type == "placing":
        ...
        end_z = target_z + 5  # Position slightly above the target for stacking
...
def generate_robotic_data(num_placing_samples=5, num_stacking_samples=5):
    ...
    dataset.push_to_hub("jan-hq/Pick-Place-Table-Reasoning-local-pos", split="train")
    return data_samples

def main():
    parser = argparse.ArgumentParser(description='Generate synthetic robotic task data.')
    parser.add_argument('--placing', type=int, default=150000, help='Number of placing task samples')
    parser.add_argument('--stacking', type=int, default=150000, help='Number of stacking task samples')
